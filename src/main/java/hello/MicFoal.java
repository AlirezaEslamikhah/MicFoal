...
public class MicFoal extends AbstractClassifier implements MultiClassClassifier, CapabilitiesHandler {
	...
	public  int _truelabel;
	public  int _predictedlabel;
	...
	public double[] PeriodicEvaluate() 
	{
		...
		System.out.println("----------------------------------------\n");
		System.out.println("The precisions are:" + precisions);
		System.out.println("The recalls are:" + recalls);
		double AvF1 = 0.0;
		double CBA;
		double mGM = 0.0;
		double CEN;
		for (int i = 0; i < this.numberOfClasses; i++) {
			AvF1 = AvF1 + ((2 * precisions.get(i) * recalls.get(i)) / (precisions.get(i) + recalls.get(i)));
		}
		AvF1 = AvF1 / this.numberOfClasses;
		System.out.println("the AvF1 is :" + AvF1);

		for (int i = 0; i < this.numberOfClasses; i++) {
			mGM = mGM * recalls.get(i);
		}
		mGM = Math.pow(mGM, 1.0/this.numberOfClasses);
		System.out.println("the Mgm is :" + mGM);
	...
	protected double CalculateWeight(Instance instance) {
	    ...
	    if(count.length>0){ //if count.length equals 0, this instance can not be predicted, so predictedLabelIndex = -2
			...
			Double temp = Double.valueOf(count[predictedLabelIndex]);
	...
	protected boolean HybridStrategy(Instance inst) {
    	...
    	{
    		predictedLabelIndex = Utils.maxIndex(count);
//			System.out.println("The predicted label is : " + predictedLabelIndex +"and the true label is : "+  trueLabelIndex);
			Double temp = Double.valueOf(count[predictedLabelIndex]);
			_predictedlabel = predictedLabelIndex; //added
		...
		System.out.println( "the predicted is " + predictedLabelIndex + "and the true label is " + trueLabelIndex);
